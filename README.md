...
The helper scripts require a working `R` installation along with the following packages:
- `Rapp`
- `tidyverse`
- `ape`
...
### GWAS 
To run GWAS use:
`julia bassgwas-run.jl --pfile [patterns_centred.csv] --ufile [U.csv] --sfile [S.csv] --obsfile [obs.csv] --odir [OUTPUT_DIRECTORY] --batchsz [BATCH_SIZE]`. This will write posterior draws and the experimental design to the output directory. Setting `--batchsz 0` will disable experimental design and run gwas only.
### Experimental design
...
`julia bassgwas-design.jl --pfile [patterns_centred.csv] [U.csv] --sfile [S.csv] --obsfile [obs.csv] --b_draws [b_draws.csv] --u_draws [u_draws.csv] --par_draws [par_draws.csv] --ofile [output_file.txt] --batchsz [batch_size]` 
